...
const mistralClient = new MistralClient(functions.config().mistral.key)
...
app.post("/translate", async (req, res) => {
  try {
    ...
    for (const lang of targetLanguages) {
      ...
      translations[lang] = chatResponse.choices[0].message.content
    }
...
app.post("/generate-proposal", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      proposal: chatResponse.choices[0].message.content,
    })
  } catch (error) {
    res.status(500).json({ error: error.message })
  }
...
app.post("/process-email", async (req, res) => {
  try {
    ...
    const summaryCompletion = await mistralClient.chat({
        model: 'mistral-small-latest',
        messages: [{role: 'user', content: `Summarize the request the client is making in this email: ${emailContent}`}],
      });

    const responseCompletion = await mistralClient.chat({
        model: 'mistral-small-latest',
        messages: [{role: 'user', content: `Write a brief email response to the message below, letting the client know we are working on their request and suggesting anything the client might do to resolve the issue themselves: ${emailContent}`}],
      });
...
app.post("/categorize-event", async (req, res) => {
  try {
    ...
    const chatResponse = await mistralClient.chat({
        model: 'mistral-small-latest',
        messages: [{role: 'user', content: `Based on the title "${eventTitle}" and description "${eventDescription}" of this calendar event, is it more likely to be a Personal or Business event? Respond with only "Personal" or "Business"`}],
      });

    res.json({
      success: true,
      category: chatResponse.choices[0].message.content.trim(),
    })
  } catch (error) {
    res.status(500).json({ error: error.message })
  }
...
app.post("/sheets-automation", async (req, res) => {
  try {
    ...
    const auth = new google.auth.GoogleAuth({
      keyFile: isEmulator ? "./service-account-key.json" : undefined, // Use service account in emulator
